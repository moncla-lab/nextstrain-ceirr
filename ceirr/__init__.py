...
def split_phenotypes_excel(input_excel, phenotypes_output, sources_output):
    pd.read_excel(input_excel).to_csv(phenotypes_output, sep="\t", index=False)
    pd.read_excel(input_excel, sheet_name="Sources").to_csv(
...
def phenotypic_characterization_annotation(
    ...
    annotation_dict = {
        clean_key(row["Nextstrain Strain"]): row for row in phenotypes if row != ""
    }
    ...
    def traverse(node):
        ...
        if strain in has_annotation:
            ...
            for i, note_str in enumerate(
                strain_annotations["SourcesByIndex"].split(",")
            ):
